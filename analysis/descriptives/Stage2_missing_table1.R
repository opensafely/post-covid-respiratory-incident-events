...
stage2 <- function(cohort_name, pre_existing, covid_history, out) {

  # Load relevant data
  input <- readr::read_rds(file.path("output", paste0("input_",cohort_name,"_stage1",".rds")))
  

  # Select data depending on pre-existing respiratory condition
  if(pre_existing == "without_pre_existing"){
    input <- filter(input, pop_preexist_resp==F)
  }
  if(pre_existing == "with_pre_existing"){
    input <- filter(input, pop_preexist_resp==T)
  }
  ...
  #---------------------------------------------------------------------------#
  # 1.c. Output table merging missing and range information for covariates #
  #---------------------------------------------------------------------------#
  
  check_both <- merge(x=check_missing, y=check_range, by = "variable",all.x=TRUE)
  write.csv(check_both, file = file.path("output/not-for-review", paste0("Check_missing_range_",cohort_name, "_",pre_existing, "_",covid_history, ".csv")) , row.names=F)
  ...
  write.csv(check_dates, file = file.path("output/not-for-review", paste0("Check_dates_range_",cohort_name, "_",pre_existing, "_",covid_history, ".csv")) , row.names=F)
  ...
  input$cov_cat_consultation_rate_group <- ""
  input$cov_cat_consultation_rate_group <- ifelse(input$cov_num_consultation_rate==0, "0", input$cov_cat_consultation_rate_group)
  input$cov_cat_consultation_rate_group <- ifelse(input$cov_num_consultation_rate>=1 & input$cov_num_consultation_rate<=5, "1 to 6", input$cov_cat_consultation_rate_group)
  input$cov_cat_consultation_rate_group <- ifelse(input$cov_num_consultation_rate>=6, "6+", input$cov_cat_consultation_rate_group)
  ...
  # Populate table 1 
  covar_names <- active_analyses %>% filter(outcome == out)
  covar_names <- str_split(active_analyses$covariate_other, ";")[[1]]

  categorical_cov <- covar_names[grep("cov_cat", covar_names)]
  categorical_cov <- append(categorical_cov, c("cov_cat_age_group","cov_cat_consultation_rate_group"))
  ...
  table1 <- table1 %>%
    ...
    filter(!(Covariate == "cov_num_consultation_rate" & !Covariate_level == "Mean   "))
  ...
  print(sd(input$cov_num_consultation_rate))
  ...
  # Save table 1
  write.csv(table1_suppressed, file = file.path("output/review/descriptives", paste0("Table1_",cohort_name, "_",pre_existing, "_",covid_history,"_",out,".csv")) , row.names=F)
...
for(out in outcomes){
  if(cohort_name == "all"){
    stage2("prevax", "with_pre_existing", "with_covid_history", out)
    stage2("prevax", "without_pre_existing", "with_covid_history", out)
    stage2("prevax", "with_pre_existing", "without_covid_history", out)
    stage2("prevax", "without_pre_existing", "without_covid_history", out)
    stage2("vax", "with_pre_existing", "with_covid_history", out)
    stage2("vax", "without_pre_existing", "with_covid_history", out)
    stage2("vax", "with_pre_existing", "without_covid_history", out)
    stage2("vax", "without_pre_existing", "without_covid_history", out)
    stage2("unvax", "with_pre_existing", "with_covid_history", out)
    stage2("unvax", "without_pre_existing", "with_covid_history", out)
    stage2("unvax", "with_pre_existing", "without_covid_history", out)
    stage2("unvax", "without_pre_existing", "without_covid_history", out)

  }else{
    stage2(cohort_name, "with_pre_existing", "with_covid_history", out)
    stage2(cohort_name, "without_pre_existing", "with_covid_history", out)
    stage2(cohort_name, "with_pre_existing", "without_covid_history", out)
    stage2(cohort_name, "without_pre_existing", "without_covid_history", out)
  }
}
